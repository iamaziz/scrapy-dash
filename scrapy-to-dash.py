...
def update_db(name, typ, path):
    ...
    try:
        ...
        if dbpath is None and dbname is None:
            ...
            print(
                'DB add >> name: {0} | type: {1} | path: {3}'.format(name, typ, path))
        else:
            print("record exists")
    except:
        pass
...
def add_urls():
    ...
    pages = {
        ...
        'Guide': 'http://doc.scrapy.org/en/latest/intro/overview.html'
    }
    ...
    for p in pages:
        ...
        for a in soup.findAll('a'):  # , class_='reference internal'):
            ...
            if len(name) > 2:
                ...
                filtered = ('index.html', 'http', '//', 'irc:',
                            'spiders.html', 'benchmarking.html', 'shell.html')
                dirpath = ['Command', 'Guide']
                ...
                if '#' in path and len(path) > 2 and not path.startswith(filtered):
                    ...
                    if 'intro/overview.html../' in path:
                        path = path.replace('intro/overview.html../', '')
...
def add_infoplist():
    ...
    info = " <?xml version=\"1.0\" encoding=\"UTF-8\"?>" \
           ...
           "</plist>".format(name, name, name,
                             'doc.scrapy.org/en/latest/' + 'index.html')
